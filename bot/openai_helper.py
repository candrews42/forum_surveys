...
class OpenAIHelper:
    ...
    async def get_chat_response(self, chat_id: int, query: str) -> tuple[str, str]:
        ...
        if self.config['show_usage']:
            answer += "\n\n---\n" \
                      f"💰 {str(response.usage.total_tokens)} {localized_text('stats_tokens', bot_language, translations)}" \
                      f" ({str(response.usage.prompt_tokens)} {localized_text('prompt', bot_language, translations)}," \
                      f" {str(response.usage.completion_tokens)} {localized_text('completion', bot_language, translations)})"
    ...
    async def get_chat_response_stream(self, chat_id: int, query: str):
        ...
        plugin_names = tuple(self.plugin_manager.get_plugin_source_name(plugin) for plugin in plugins_used)
        if self.config['show_usage']:
            answer += f"\n\n---\n💰 {tokens_used} {localized_text('stats_tokens', self.config['bot_language'], self.config['translations'])}"
            if show_plugins_used:
                answer += f"\n🔌 {', '.join(plugin_names)}"
        elif show_plugins_used:
            answer += f"\n\n---\n🔌 {', '.join(plugin_names)}"
    ...
    async def __common_get_chat_response(self, chat_id: int, query: str, stream=False):
        ...
        try:
            ...
            raise Exception(f"⚠️ _{localized_text('openai_invalid', bot_language, translations)}._ ⚠️\n{str(e)}") from e
    ...
    async def generate_image(self, prompt: str) -> tuple[str, str]:
        ...
        try:
            ...
            if len(response.data) == 0:
                ...
                raise Exception(
                    f"⚠️ _{localized_text('error', bot_language, translations)}._ "
                    f"⚠️\n{localized_text('try_again', bot_language, translations)}."
                )
    ...
    async def transcribe(self, filename, model: str = "whisper-1"):
        ...
        try:
            ...
            logging.exception(e)
            raise Exception(f"⚠️ _{localized_text('error', self.config['bot_language'], self.config['translations'])}._ ⚠️\n{str(e)}") from e
